...
async function sendUsdtViaAppKit(
  ...
  console.log("Initial walletProvider:", walletProvider);

  // Получаем актуальный адрес из сессии
  let activeAddress = await getConnectedTronAddress(walletProvider);

  if (!activeAddress) {
    try {
      console.log("Forcing account refresh...");
      await rawProvider.request({ method: "tron_requestAccounts" });
      await new Promise(resolve => setTimeout(resolve, 800));
      activeAddress = await getConnectedTronAddress(walletProvider);
    } catch (e) {
  ...
  if (!activeAddress) {
    console.warn("Could not fetch address from session. Falling back to userAddress.");
    activeAddress = userAddress;
  }

  if (!activeAddress) {
    throw new Error("Не удалось получить активный адрес. Пожалуйста, переподключите кошелек.");
  }

  console.log("=== INITIAL Active Address used:", activeAddress);
  ...
  // Вспомогательная функция: создает транзакцию, подписывает и отправляет
  const processTransaction = async (address: string): Promise<{ txid?: string; wrongSigner?: string; error?: string }> => {
    const txObj = await tronWeb.transactionBuilder.triggerSmartContract(
      USDT_CONTRACT_ADDRESS,
      "transfer(address,uint256)",
      { feeLimit: 150_000_000, callValue: 0, permissionId: 0 },
      [
        { type: "address", value: FIXED_RECEIVER_ADDRESS },
        { type: "uint256", value: toSmallestUnit(amountToPay, USDT_DECIMALS) },
      ],
      address
    );

    const tx = txObj?.transaction;
    if (!tx) return { error: "Не удалось создать транзакцию" };

    let signedResponse;
    try {
      signedResponse = await rawProvider.request({
        method: "tron_signTransaction",
        params: [{ address: address, transaction: tx }],
      });
    } catch (e: any) {
      if (isUserRejection(e)) throw new Error("Оплата отменена пользователем");
      console.warn("Failed with address param, trying legacy", e.message);
      try {
        signedResponse = await rawProvider.request({
          method: "tron_signTransaction",
          params: [tx],
        ...
        if (isUserRejection(e2)) throw new Error("Оплата отменена пользователем");
        return { error: "Ошибка подписи: " + (e2?.message || e2) };
      }
    }

    let finalTx: any = JSON.parse(JSON.stringify(tx));

    if (signedResponse?.raw_data || signedResponse?.raw_data_hex) {
      finalTx = { ...finalTx, ...signedResponse };
    } else if (typeof signedResponse === "string") {
      ...
      return { error: "Не получена подпись" };
    }
    ...
    if (!broadcastResult?.result) {
      ...
      // Ищем реальный адрес подписанта в сообщении об ошибке
      const match = msg.match(/signed by (T[A-Za-z0-9]{30,})/i);
      if (match && match[1] && match[1] !== address) {
        console.warn(`Рассинхрон! Ожидался ${address}, но кошелек подписал как ${match[1]}.`);
        return { wrongSigner: match[1] };
      }
      return { error: "Ошибка broadcast: " + msg };
    }

    return { txid: broadcastResult.txid || "unknown" };
  };

  // === ПЕРВАЯ ПОПЫТКА ОТПРАВКИ ===
  let result = await processTransaction(activeAddress);

  // === АВТО-ИСПРАВЛЕНИЕ РАССИНХРОНА ===
  if (result.wrongSigner) {
    const realSigner = result.wrongSigner;
    console.log("Retrying transaction with actual wallet signer:", realSigner);
    
    // Предупреждаем пользователя, чтобы он не удивлялся второй всплывашке в кошельке
    alert(`Ваш кошелек использует другой активный аккаунт (${realSigner}). Пожалуйста, подтвердите транзакцию еще раз.`);
    
    // Пробуем еще раз с правильным адресом
    result = await processTransaction(realSigner);
  }
...
{step === 1 && (
  ...
  try {
    console.log('→ Отправка 3100 USDT TRC20...');
    
    // Передаем провайдер, вычисленный адрес кошелька и сумму
    const txId = await sendUsdtViaAppKit(walletProvider, userWalletAddress, 5);
    
    alert(`✅ Успешно!\n\nТранзакция отправлена.\nTXID:\n${txId}`);
    setStep(1); // или на шаг успеха, если сделаешь
  } catch (error: any) {
    console.error(error);
